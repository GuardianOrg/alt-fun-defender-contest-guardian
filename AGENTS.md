...
The combination is what works: the seed buy lands ahead of the gate, and the 3-block delay stops anyone (sniper or retail) racing the seed at block N or piling in at N+1..N+3. Trading opens at `launchBlock + LAUNCH_TRADING_DELAY_BLOCKS + 1`. The seed buy itself bypasses the delay via a transient-storage flag set inside `Bonding.launch` and consumed by the very next `Bonding.buy` in the same tx — so the in-tx seed always lands while same-block sniper bundles (separate txs, transient cleared) revert with `TradingNotOpen`. The delay gates buys only; sells are not delayed, so a creator can withdraw the seed within the window (they are never forced to leave it in — same rationale as the uncapped seed below).
