...
- **Retired LTs keep trading but freeze (accepted).** BounceTech can de-register an LT (it redeploys a fresh one at a new address and flips the old address's `ltExists` to `false`). The old contract isn't destroyed — `mint` / `redeem` / `exchangeRate` keep working — so bonding curves already pointing at it keep trading and stay sellable. What changes is that the rate stops tracking the underlying once keepers stop running positions against the old LT: leverage is effectively frozen and the USD graduation trigger (`raisedLT × exchangeRate`) can't ripen further. The supply-sellout trigger still graduates such tokens, and holders can always exit via `redeem`, so no funds are stranded — the curve just behaves like a stable vault from that point on. New launches against a retired LT are already rejected by `Bonding.launch`'s `ltExists` gate, so only pre-existing bonding curves are ever affected and the blast radius only shrinks.
...
Failure mode is **fail-closed**: missing `OPENAI_API_KEY`, OpenAI 4xx/5xx, network timeout, or malformed response all surface as a 503 ("moderation temporarily unavailable") and **no upload happens**. Letting unmoderated content into R2 is the failure mode this layer exists to prevent. Two PNG encodings that OpenAI provably can't decode are caught locally and answered with a 400 asking for a different image, since retrying the same bytes can never pass — see [`apps/api/AGENTS.md`](apps/api/AGENTS.md) → *Undecodable PNG encodings*.
