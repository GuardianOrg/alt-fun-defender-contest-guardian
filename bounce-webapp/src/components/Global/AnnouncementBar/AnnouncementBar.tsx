...
import { LIQUIDATION_SCORE_ROUTE, REGISTER_ROUTE } from "../../../app/routes";
...
const AnnouncementBar = () => {
  ...
  /* liquidation score */
  if (
    location.pathname !== `/${LIQUIDATION_SCORE_ROUTE}` &&
    ...
    return (
      <>
        <div className={`${styles.announcementBar} ${styles.primary}`}>
          ...
          <Link to={`/${LIQUIDATION_SCORE_ROUTE}`} className={styles.link}>
            claim your Liquidation Score
          </Link>{" "}
          now. 🥳
        </div>
      </>
