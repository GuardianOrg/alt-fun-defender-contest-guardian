...
const tokenSchema = {
  ...
  properties: {
    ...
    volume24hUsd: { type: "number", nullable: true, description: "Total USD routed through `LaunchpadRouter` for this token in the last 24h (buys + sells). `0` = no trades in the window (legitimately quiet); `null` = indexer aggregation unavailable or truncated (unknown). Used as an input to the trending score." },
    totalVolumeUsd: { type: "number", nullable: true, description: "Lifetime gross USD routed through `LaunchpadRouter` for this token (buys + sells, never subtracts). Sourced from the indexer's running counter on the `token` row so it's not affected by pagination truncation. `null` only when the indexer is unreachable; `0` when the token has never traded." },
    lastTradeAt: { type: "string", format: "date-time", nullable: true, description: "Timestamp of the most recent `LaunchpadRouter` trade within the 24h window. `null` means either no trades in that window or indexer unavailable — disambiguate using `volume24hUsd`." },
