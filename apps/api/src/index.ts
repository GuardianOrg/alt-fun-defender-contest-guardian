...
import { WsIpLimiter, tryAcquireIpSlot } from "./websocket/ip-limiter.js";
...
app.get("/ws", async (c) => {
  ...
  // Stamp the shard key on the URL so the DO knows its own subject (used
  // for log context — `idFromName` is one-way).
