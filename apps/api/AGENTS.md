...
A lock qualifies only when it is a non-cancelable Sablier pure timelock with more than `MIN_LOCK_DURATION_SECONDS` (7 days) still to run, **and** the token's qualifying locks add up to at least `MIN_LOCK_PERCENT` (10%) of supply. The indexer already guarantees the first two properties — see `apps/indexer/AGENTS.md` → *Supply locks* — so `lib/token-locks.ts` applies the duration cutoff, sums deposits, and drops tokens under the share floor. `summariseTokenLocks` re-applies the cutoff that `fetchActiveTokenLocks` pushes into SQL: the SQL predicate is there to keep the scan small, the TypeScript one is the definition.

`MIN_LOCK_PERCENT` exists because **nobody has to be the creator to lock a token.** The handler never checks the stream's `sender` or `funder` — deliberately, since a whale locking 75% is a real lock — which means without a floor anyone holding dust of any token could escrow it for eight days and hang a `LOCKED` pill on someone else's launch for the price of gas. At 10% the vector inverts: an attacker has to buy and give up 100M tokens for over a week, which is a favour to the token. The floor is applied to the **per-token total**, so two 6% locks still count as a 12% lock; that's why it can't become a `deposit_amount` predicate in SQL alongside the cliff-time one.
...
**`Cache-Control: s-maxage` does NOT admit a Worker response to Cloudflare's zone cache** — the zone reads `Cloudflare-CDN-Cache-Control` instead, and only when `cache.enabled` is set in `wrangler.json`. Nine routes declaring `s-maxage` were measured in production returning no `cf-cache-status` header at all.

Every cache directive in this app is therefore generated by `src/utils/cache-control.ts`, which stamps all three tiers from one TTL:

| Tier | Header | Directive |
|---|---|---|
| Browser | `Cache-Control` | `max-age=0` — revalidate every reload. Dropping this let browsers heuristically cache and froze the home-page list until users cleared browsing data. |
| Worker (`caches.default`) | `Cache-Control` | `s-maxage=<ttl>` — the Workers Cache API evicts strictly on this, which is what `putWithSwr` keys off. |
| Cloudflare zone | `Cloudflare-CDN-Cache-Control` | `max-age=<ttl>, stale-while-revalidate=<2×ttl>`. Uses `max-age` because a present `s-maxage` disables Cloudflare's own stale-while-revalidate. |

Use `setEdgeCacheHeaders(c, ttl)` when the handler stamps headers before building the response, `applyEdgeCacheHeaders(response, ttl)` when it has the response in hand, and `setImmutableAssetHeaders(c)` for content-addressed blobs. Pass an explicit stale window as the third argument when it isn't the 2× default (`/tokens/:addr/meta`, `/assets/leveraged-tokens`).

**Caching is opt-in, and the default is `no-store`.** `cache.enabled` is a Worker-wide setting — it cannot be scoped to the routes that want caching — so `middleware/default-no-store.ts` runs outermost and stamps `no-store` on both headers for any response that declared no policy of its own. Without it, a headerless response is left to whatever default the platform applies, and `GET /api/v1/admin/api-keys` (gated by `adminAuth` *inside* the admin router) could be re-served from a URL-keyed zone entry to a caller with no `X-Admin-Key`.

Never hand-write a `Cache-Control` string with a positive TTL — `src/__tests__/cache-header-coverage.test.ts` fails the build if you do. The one allowed literal is the zeroed `private, no-store, max-age=0, s-maxage=0` opt-out on the wallet-aware branch of `routes/tokens/detail.ts`.
...
| `graduating` | Non-graduated tokens whose enriched `curveFilled` ≥ **75** | `curveFilled desc` (tie-break `mcapUsd desc`) — `sort=` / `dir=` ignored | Ponder pool of `STATUS_POOL_SIZE` ordered by `curveSupply asc`, filtered + sorted in memory after enrichment |
| `graduated` | Ponder `graduated: true` | `graduatedAt desc` from Ponder | Ponder-first |

**The 75% gate is a *curve filled* signal, distinct from the per-token `status` field.** `EnrichedToken.status` (and the `pendingGraduation` flag) continues to be driven by on-chain lifecycle (`Lifecycle.Curve` → `Lifecycle.Graduating` → `Lifecycle.Graduated`, mirroring the `Bonding.sol` enum) — the GRADUATING pill + trade-panel overlay on the frontend key off `status === "graduating"`, which means "phase 1 has fired, trades frozen", *not* "≥75% curve filled". The two concepts are deliberately decoupled: a token at 82% curve filled but not yet in phase 1 shows up in the tab (it's close) but doesn't show the pill or block trades.
...
- `reserve1` is initialised to `virtualLtAtLaunch` (= `$3K` worth of LT at launch-time `rate_at_launch`) and grows with buys — not 0 at launch.
