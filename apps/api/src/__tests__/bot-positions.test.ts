 ...
 * chain). For tests that only care about the cost-basis / PnL math,
 * default the chain balance to the router-tracked balance — i.e. no
 * out-of-router disposal — by deriving it from the position rows. Tests
 * that exercise phantom filtering override `tokenBalances` directly.
 */
const positionsWithMatchingChainBalances = (rows: PositionRow[]) => ({
  walletBotPositions: { items: rows },
  tokenBalances: {
    items: rows
...
describe("GET /bot/positions/:wallet", () => {
  ...
  it("ignores malformed rows without crashing the response", async () => {
    mockPonderQuery.mockResolvedValue({
      ...
      tokenBalances: {
        items: [{ tokenAddress: TOKEN_A, balance: "1000" }],
      },
    });
    const res = await createApp().request(
      `/bot/positions/${VALID_WALLET}`,
  ...
  it("clamps balance and rescales value when chain balance is below router-tracked balance", async () => {
    ...
    mockPonderQuery.mockResolvedValue({
      ...
      tokenBalances: {
        items: [{ tokenAddress: TOKEN_A, balance: "1000000000000000000" }],
      },
    });
    const res = await createApp().request(
      `/bot/positions/${VALID_WALLET}`,
