...
const OVERVIEW_HTML = [
  ...
  "/settings — slippage, default buy amount, MEV, confirmations",
  "/security — PIN, anti-phishing phrase, withdrawal whitelist",
  ...
  "",
  "<b>Security tips</b>",
  "• AltFunBot will <b>never</b> ask for your seed phrase or private key via DM.",
  "• Never search for AltFunBot in Telegram. Use only the link from <a href=\"https://alt.fun\">alt.fun</a>.",
...
const WALLET_HTML = [
  ...
  "<b>Wallets</b>",
  "",
  "Each Telegram account can hold up to 10 wallets on AltFunBot. One is always <i>active</i> — buys, sells, and withdrawals use it as the signer.",
  "",
...
const TRADING_HTML = [
  ...
  "",
  "<b>/sell &lt;contract&gt; [%|amount]</b>",
  "Sell a position by percentage (25 / 50 / 75 / 100%) or absolute token amount. Sells go through the BounceTech LT redemption path — if the LT's USDC buffer is depleted the trade can revert with <code>InsufficientBalance</code>; sell in smaller chunks and retry after ~10 seconds.",
  "",
...
const FEES_HTML = [
  ...
  "",
  "• <b>Bot fee 0.5%</b> — charged by AltFunBot. If you came in via a referral link, 0.1% of your trade is paid to your referrer's rewards wallet; the rest goes to the bot treasury.",
  "• <b>Alt Fun fee 0.5%</b> — charged by the alt.fun protocol. Split 0.4% protocol / 0.1% to the token creator.",
  "",
  "Post-graduation trades also pay HyperSwap's 0.3% LP fee on top, paid to HyperSwap liquidity providers (AltFunBot takes 0% of this).",
  "",
...
const PNL_HTML = [
  ...
  "• AltFunBot fee (0.5%, of which 0.1% is paid to your referrer if you have one)",
...
const SECURITY_HTML = [
  ...
  "<b>Security</b>",
  "",
  "AltFunBot will <b>never</b> ask for your seed phrase or private key via DM, and will never ask you to log in with a phone number or QR code.",
  "",
  "<b>Protect your account</b>",
  "• Only use the AltFunBot link from <a href=\"https://alt.fun\">alt.fun</a>. Never search for the bot inside Telegram — copycats are everywhere.",
  ...
  "",
  "<b>PIN</b>",
  "Set a 6-digit PIN in /security. The PIN gates withdrawals, private key exports, wallet deletion, and rewards-wallet changes. After 5 wrong attempts you're locked out for 30 minutes. If you forget your PIN, request a reset in /security — the new PIN unlocks after a 24-hour delay (the delay is what protects you from a stolen Telegram session draining your funds).",
  "",
  "<b>Withdrawal protections</b>",
  "Enable the withdrawal address whitelist in /security to block sends to any address you haven't pre-approved. Once enabled, disabling it requires a 24-hour cooldown.",
].join("\n");
...
const WITHDRAW_HTML = [
  ...
  "2. If the withdrawal whitelist is enabled in /security, non-whitelisted destinations are rejected",
  "3. PIN prompt",
