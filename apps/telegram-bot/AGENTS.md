...
**Background polling for pending receipts.** `awaitReceipt` in `lib/trade.ts` is bounded by `RECEIPT_TIMEOUT_MS` (20s) so the webhook handler ACKs Telegram before Telegram retries. When the in-band wait expires the trade returns `{ ok: false, kind: "pending", txHash }` and `runWithTxStatusUpdates` edits the bubble to "⏳ Tx pending — still polling in the background; this message updates once mined." The system does **not** stop there: `schedulePendingTxPoll` (`lib/pending-tx-poller.ts`) writes the tx into the chat's `ChatDO` storage and arms the DO alarm. The alarm handler (`ChatDO.alarm` → `processPendingTxAlarm`) re-polls `getTransactionReceipt` every `PENDING_TX_POLL_INTERVAL_MS` (5s) until the chain settles the tx — at which point it edits the same bubble in place with the final receipt-confirmed or reverted copy, and `markFinal`s the idempotency commit-log so any retry sees the recorded outcome instead of resubmitting. Polling stops after `PENDING_TX_MAX_POLL_DURATION_MS` (30 min), leaving the bubble at "⏳ pending — receipt not seen within 30 minutes" with the explorer link as the user's last affordance. The alarm path uses a raw `fetch` against the Telegram Bot API (no grammY `Bot` instance) because there is no `Context` / session / middleware in scope. The post-trade `/start` prompt is intentionally **not** re-fired from the alarm path — the user has long since moved on by the time a delayed receipt lands, and a stale home menu beneath the bubble would just be chat clutter.
...
falls back to a text-only `sendMessage` with the full 5-trade body, so
...
- Settable via [Change rewards wallet] → opens a picker that renders one button per existing custodial wallet (two-per-row; label when set, otherwise the shortened address `0x5A2e...F444`), followed by a dedicated `Custom` row that enters the address-entry wizard for any HyperEVM address (does not have to be a bot custodial wallet). Both paths are PIN-gated. Tapping a wallet button skips the address-entry step and goes straight to the PIN prompt; tapping `Custom` proceeds through the legacy address-entry + burn-address-warning wizard. The shortened-address format mirrors the wallets api description: `0x` + first 4 hex chars + `...` (three ASCII dots) + last 4 hex chars.
...
**There is no `/cancel` text command.** Every wizard prompt carries Back/Home, so the legacy `isCancel` helpers and "send /cancel to abort" copy have been removed entirely (the previous compat fallback is gone). Slash commands typed mid-wizard halt via `isOtherSlashCommand` + `haltAndForward` in `lib/conversation-commands.ts`, which now also catches `/cancel` — it simply produces no acknowledgment because no `/cancel` handler exists. New wizards must not introduce a `/cancel` text path.
...
**No dead-end bubbles.** Every bot reply must leave the user with a tappable way out — either a forward-action keyboard that includes `backHomeRow()`, or `backHomeMarkup()` on its own when there is nothing else to offer. This covers outage cards ("Data temporarily unavailable — try again in a moment."), token-not-found replies, "no active wallet" notices, invalid-input retries, balance-verification failures — every error or notice the bot sends as a `sendMessage` (not a callback toast). The previous bug was sending these as plain `ctx.reply(text)` with no markup, stranding users who then had to retype `/start` to escape. Route them through `replyWithNav(ctx, text, extra?)` so the nav row is attached by default. Callback toasts (`answerCallbackQuery({ text, show_alert: true })`) are exempt — Telegram does not render inline keyboards on toasts, so the toast itself is the affordance, and the underlying bubble (which the user came from) still carries its own keyboard.

**The narrow exception: audit-grade info that must persist.** Receipts, tx hashes, security-sensitive confirmations ("Wallet deleted.", "PIN changed.", export key reveals, withdrawal receipts) intentionally do not edit-in-place — they land as a fresh bubble and stay in the chat log so the user can scroll back to them as a record. These replies do not need the nav row on the bubble itself (it would clutter the audit record), but they MUST be followed by a fresh `/start` view (via `sendStartPromptAfterTrade` in `commands/start.ts` or the equivalent) so the user is re-seated on the home menu and never left in nav-less limbo. The audit bubble is the record; the start prompt is the escape. Without the trailing start prompt this rule is violated and the bubble becomes a dead end.

**Refresh-on-Back via the view registry.** Snapshots pushed onto `session.navStack` capture text + keyboard at push time, but the parent screen's state can move while the user is in a deeper view (a wallet was just created, a preset was just edited, a balance just refreshed). Restoring the frozen capture on Back surfaces stale data and, worse, makes the just-made change appear undone. The fix: each navigable parent registers a builder via `registerView(id, ctx => SubmenuView | null)` in `lib/nav.ts`. When a forward nav fires `editToSubmenu(ctx, view)`, the helper tags the pushed parent snapshot with the bubble's current view id (read from `session.navCurrentView`, set by the most recent `editToSubmenu`/`setCurrentView` call). On Back, the handler invokes the registered builder and renders the fresh output, falling back to the frozen text/keyboard only when the builder is missing or returns `null` (RPC degraded, no active wallet). Currently registered: `start`, `wallet:main`. When migrating a new view, (a) call `registerView` at module init, (b) pass `view: { id: "<your-view-id>" }` on the `SubmenuView` you hand to `editToSubmenu` from the entry point, (c) for commands that open a fresh bubble via `ctx.reply` (not `editToSubmenu`), follow up with `setCurrentView(ctx.session, { id: "<your-view-id>" })` so a forward nav from that bubble still tags its parent. Wizard text-input prompts opt out by leaving `view` unset on their submenu payload — they have no fresh-data builder to call.

**Wizard text-input prompts must carry the nav row on the prompt itself**, not just on the parent menu. Telling the user to "Tap Home to exit" while the only Home button lives on a parent menu scrolled off-screen is the original UX bug this section exists to prevent. Inside a `conversation.waitFor("message:text")` flow, use `replyWithNav(ctx, text, extra?)` from `lib/nav.ts` instead of `ctx.reply` — the helper defaults `reply_markup` to `backHomeMarkup()` (`[← Back] [🏠 Home]` as a one-row inline keyboard) unless the caller supplies its own. Terminal replies that end the flow ("Wallet imported.", error toasts) keep using plain `ctx.reply` — they don't need the nav row and would just clutter the chat.

**Start-menu button → wizard entries edit the start bubble in place.** Every `START_CALLBACK.*` handler that navigates from `/start` into a wizard or sub-screen (Wallet, Settings, Positions, Referral, Help, Buy, Sell, Track, Withdraw) calls `editToSubmenu(ctx, …)` from `lib/nav.ts` to replace the start-bubble content with the first wizard prompt — not `ctx.reply(...)`, which would leave the stale start menu visible above a freshly-sent prompt below it. For wizards that run as grammY conversations (`/buy`, `/sell`, `/track`, `/withdraw`), the callback captures the edited bubble's `(chatId, messageId)` as a `MessageRef` and threads it into `ctx.conversation.enter(...)` as the third argument; the conversation uses `safeEditMessageById` (also in `lib/nav.ts`) to re-render that same bubble for every subsequent step (token-not-found retries, the final card / asset picker). The slash-command entry paths (`/buy <addr>`, `/sell`, `/track`, `/withdraw <args>`) have no parent bubble to edit and fall through to the legacy `replyWithNav` prompt — the conversation handler accepts the absent `MessageRef` and degrades automatically. **Buy/sell origin bubbles are NOT pushed onto the workflow stack until the final card replaces them**: the stack is swept on conversation completion and any premature push would delete the bubble the wizard is still running inside. The withdraw asset picker DOES push the origin onto the stack because the wizard's terminal output is a fresh tx receipt, not an edit of the picker.
...
| `API_KEY` | **Required.** Dedicated `X-API-Key` for the bot's calls to `apps/api`. Provision with `apps/api/scripts/provision-api-key.ts` (fleet-aggregate rate limit — the bot fans many users through one Worker IP, so the anonymous 240/min ceiling would starve under any real load), then `wrangler secret put API_KEY` per environment. `createBot` throws at construction if unset. Rotate by issuing a new row, redeploying with the new secret, then deactivating the old row. |
...
src/
  ...
  lib/
    ...
    pending-tx-poller.ts  — Background pending-receipt poller: persists in-flight txs into `ChatDO` storage, alarm re-polls `getTransactionReceipt` until the chain settles or the 30-min cap fires, edits the original receipt bubble in place when the result lands
