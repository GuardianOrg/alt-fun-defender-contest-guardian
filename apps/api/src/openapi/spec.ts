...
function successResponse(
  ...
  if (dataSource) {
    props["dataSource"] = {
      ...
      enum: ["live", "degraded"],
      description:
        "Indicates Ponder indexer availability. 'degraded' means on-chain data may be stale.",
    };
  }
...
const tokenSchema = {
  ...
  properties: {
    ...
    graduated: {
      type: "boolean",
      description:
        "Whether the token has graduated to HyperSwap. Derived from the on-chain curve state.",
    },
    graduatedAt: { type: "string", format: "date-time", nullable: true },
    poolAddress: {
      type: "string",
      nullable: true,
      description: "HyperSwap V2 pair address after graduation",
    },
    bondingPair: {
      type: "string",
      nullable: true,
      description:
        "Bonding-curve pair contract address. `null` while Ponder is unavailable.",
    },
    hyperswapPair: {
      type: "string",
      nullable: true,
      description:
        "HyperSwap V2 pair address after graduation (duplicate of `poolAddress`, kept for clients keyed on the on-chain field name).",
    },
    priceUsd: {
      type: "number",
      nullable: true,
      description:
        "Current token price in USD. `null` when Ponder or BounceTech is degraded — treat as unknown, never zero.",
    },
    mcapUsd: {
      type: "number",
      nullable: true,
      description:
        "Current fully-diluted market cap in USD (`priceUsd × 1B`). `null` under the same degraded conditions as `priceUsd`.",
    },
    change24h: {
      type: "number",
      nullable: true,
      description:
        "24h price change as a percentage. For tokens younger than 24h this is the since-launch delta. `null` when the reference price is unavailable.",
    },
    ltChange24h: {
      type: "number",
      nullable: true,
      description:
        "24h percentage change of the backing LT's exchange rate (independent of any curve / router activity). `null` when BounceTech has no rate at either end of the window.",
    },
    volume24hUsd: {
      type: "number",
      nullable: true,
      description:
        "Total USD routed through `Zap` for this token in the last 24h (buys + sells). `0` = no trades in the window (legitimately quiet); `null` = indexer aggregation unavailable (unknown). Sole signal behind `?sort=trending`.",
    },
    totalVolumeUsd: {
      type: "number",
      nullable: true,
      description:
        "Lifetime gross USD routed through `Zap` for this token (buys + sells, never subtracts). Sourced from the indexer's running counter on the `token` row so it's not affected by pagination truncation. `null` only when the indexer is unreachable; `0` when the token has never traded.",
    },
    lastTradeAt: {
      type: "string",
      format: "date-time",
      nullable: true,
      description:
        "Timestamp of the most recent `Zap` trade within the 24h window. `null` means either no trades in that window or indexer unavailable — disambiguate using `volume24hUsd`.",
    },
    twitterUrl: {
      type: "string",
      description:
        "Twitter / X handle (no `@`, no URL prefix). Frontend builds the link as `https://x.com/<handle>`. Empty string when the creator did not set one or supplied a value that failed sanitisation. Pre-#400 rows may still hold legacy URLs; clients should pass the value through `buildTwitterUrl` before rendering.",
    },
    telegramUrl: {
      type: "string",
      description:
        "Telegram path component (`<username>`, `+<invite>`, or `joinchat/<hash>`). Frontend builds the link as `https://t.me/<value>`. Empty string when unset or invalid. Pre-#400 rows may still hold legacy URLs; clients should pass the value through `buildTelegramUrl` before rendering.",
    },
    websiteUrl: {
      type: "string",
      description:
        "Canonical http(s) URL with multi-label ASCII hostname (no userinfo). Empty string when unset or invalid. Pre-#400 rows may still hold legacy / unvalidated values; clients should pass the value through `buildWebsiteUrl` before rendering.",
    },
    creator: { type: "string" },
    ...
    { $ref: "#/components/schemas/Token" },
    {
      ...
      properties: {
        ...
        curveFilled: {
          ...
          nullable: true,
          description:
            "Percentage of curve filled (0-100). USD-denominated: `realLt × currentRate / graduationThresholdUsd × 100`, clamped to [0, 100]. Falls back to supply-based progress when `k` / rate / `ltReserve` are unavailable so the bar stays populated during indexer or BounceTech outages. `null` when neither path can be derived (e.g. the indexer is unreachable for this token and `curveSupply` is null) — clients must render '—', never `0`.",
        },
        curveFilledOrganic: {
          type: "number",
          nullable: true,
          description:
            "Portion of `curveFilled` attributable to organic USDC buys. `null` while indexer/BounceTech are degraded or post-graduation.",
        },
        curveFilledLeverageBoost: {
          type: "number",
          nullable: true,
          description:
            "Portion of `curveFilled` attributable to LT price appreciation. Clamped at 0 when the LT has dropped (we never surface a negative boost on the UI).",
        },
        curveRaisedUsd: {
          type: "number",
          nullable: true,
...
const tradeSchema = {
  ...
  properties: {
    ...
    tokenAmount: {
      type: "string",
      description: "Token amount in raw units (18 decimals)",
    },
...
const candleSchema = {
  ...
  properties: {
    time: {
      type: "integer",
      description: "Unix timestamp of the candle bucket",
    },
    open: { type: "number" },
    high: { type: "number" },
    low: { type: "number" },
...
`,
  ...
  tags: [
    {
      name: "Tokens",
      description: "Token listing, search, details, and creation",
    },
    {
      name: "Trades",
      description: "Trade history, OHLCV charts, and sparklines",
    },
    ...
    { name: "Admin", description: "Admin operations (requires X-Admin-Key)" },
    {
      name: "Moderation",
      description:
        "Wallet-signed moderation actions (token hide / unhide). Auth via EIP-191 signature recovered to an address in the admin allowlist — see `routes/moderation.ts`.",
    },
  ],
  components: {
    schemas: {
    ...
    securitySchemes: {
      ApiKeyAuth: {
        ...
        name: "X-API-Key",
        description:
          "API key for rate-limited access to all /api/v1/* endpoints",
      },
      AdminKeyAuth: {
        type: "apiKey",
        in: "header",
  ...
  paths: {
    ...
    "/api/v1/tokens": {
      get: {
        ...
        summary: "List tokens",
        description:
          "Returns a paginated list of tokens with optional filters for underlying asset, status, direction, leverage, and creator.",
        parameters: [
          ...paginationParams,
          {
            name: "underlying",
            in: "query",
            schema: { type: "string", enum: UNDERLYING_ENUM },
            description:
              "Filter by underlying asset. The `xyz:` namespace covers BounceTech's equity / commodity perps (S&P 500, NVDA, Gold, etc.); the rest are Hyperliquid spot/perps.",
          },
          {
            name: "status",
            in: "query",
          ...
          },
          {
            ...
            description:
              "Return only tokens created strictly after this ISO-8601 timestamp. Cursor-style backfill: a client tracking the most recent `createdAt` it's processed can pass that value here to receive everything newer without re-receiving the boundary row.",
          },
          {
            name: "sort",
            in: "query",
        ...
        responses: {
          "200": {
            description:
              "List of tokens with on-chain enrichment (curve state, USD raised, USD-denominated graduation progress, organic-vs-leverage split). Same shape as `GET /api/v1/tokens/{address}`.",
            content: {
              "application/json": {
          ...
          "400": {
            description: "Invalid pagination or `createdAfter` parameter",
      ...
      post: {
              ...
              schema: {
                ...
                properties: {
                  address: {
                    ...
                    description:
                      "Token contract address (returned by `Zap.TokenCreated`).",
                  },
                },
              },
            },
        ...
        responses: {
          "200": {
            description: "Token already registered (returns the existing row)",
            content: {
              "application/json": {
                schema: successResponse({ $ref: "#/components/schemas/Token" }),
              },
            },
          },
          "201": {
          ...
          "404": {
            description: "Token not found on-chain",
            content: { "application/json": { schema: errorResponse } },
          },
          "422": {
            description: "Image URL or LT failed validation",
            content: { "application/json": { schema: errorResponse } },
          },
          "500": {
            description:
              "Internal error during registration (e.g. DB write race that couldn't be re-resolved)",
            content: { "application/json": { schema: errorResponse } },
          },
          "502": {
            description: "Upstream RPC or BounceTech directory unavailable",
            content: { "application/json": { schema: errorResponse } },
          },
        },
      },
    ...
    "/api/v1/tokens/search": {
      get: {
        ...
        description:
          "Search tokens by name, ticker, or address. Returns up to 20 results.",
        parameters: [
          {
            name: "q",
            in: "query",
    ...
    "/api/v1/tokens/batch": {
      post: {
        ...
        description:
          "Fetch multiple tokens by address in a single request. Maximum 100 addresses.",
        parameters: [apiKeyHeader],
        requestBody: {
          required: true,
          content: {
    ...
    "/api/v1/trades": {
      get: {
        ...
        description:
          "Returns the most recent trades across all tokens. Supports `offset` for paginating backwards through history (newest-first ordering).",
        parameters: [
          {
            name: "limit",
            in: "query",
    ...
    "/api/v1/trades/ohlcv/{address}": {
      get: {
        ...
        responses: {
          ...
          "400": {
            description: "Invalid address or interval",
            content: { "application/json": { schema: errorResponse } },
          },
          "503": {
            description: "Indexer unavailable",
    ...
    "/api/v1/trades/sparkline/{address}": {
      get: {
        ...
        description:
          "Returns sampled price points for a mini chart. Useful for token list views.",
        parameters: [
          addressParam("address", "Token contract address"),
          {
            name: "points",
    ...
    "/api/v1/creators/{address}": {
      get: {
        ...
        description:
          "Returns creator profile, their tokens, and aggregate stats (total volume, tokens created).",
        parameters: [
          addressParam("address", "Creator wallet address"),
          apiKeyHeader,
        ],
        responses: {
          "200": {
                  ...
                  properties: {
                    profile: {
                      ...
                      properties: {
                        ...
                        twitterUrl: {
                          ...
                          nullable: true,
                          description:
                            "Twitter / X handle (no `@`, no URL prefix). Frontend builds the link via `buildTwitterUrl`.",
                        },
                    ...
                    stats: {
                      ...
                      properties: {
                        ...
                        totalVolume: {
                          type: "string",
                          description: "Total USDC volume in raw units",
                        },
                      },
                    },
                  },
                ...
                schema: successResponse({
                  ...
                  properties: {
                    positions: {
                      ...
                      items: {
                        ...
                        properties: {
                          ...
                          tokenAmount: {
                            type: "string",
                            description: "Raw token balance (18 decimals)",
                          },
                          costBasisUsdc: {
                            type: "string",
                            description:
                              "Cumulative USDC paid (6 decimals), reduced proportionally on each sell. `0` for tokens acquired only via direct Transfer.",
                          },
                        },
                      },
                    },
                    approximate: {
                      type: "boolean",
                      description:
                        "True only when the wallet holds more than 1000 distinct tokens (degenerate case). Cap exists to bound the query — callers in this regime should use on-chain `balanceOf` multicall instead.",
                    },
                  },
                }),
              },
    ...
    "/api/v1/stats": {
      get: {
        ...
        description:
          "Returns aggregate platform statistics including token counts and 24h volume.",
        parameters: [apiKeyHeader],
        responses: {
          "200": {
                ...
                schema: successResponse(
                  {
                    ...
                    properties: {
                      tokensLive: {
                        type: "integer",
                        description: "Tokens currently on bonding curve",
                      },
                      tokensGraduated: {
                        type: "integer",
                        description: "Tokens that have graduated to HyperSwap",
                      },
                      totalTokens: { type: "integer" },
                      volume24h: {
                        type: "string",
                        description:
                          "24h USDC volume in raw units (6 decimals)",
                      },
                    },
                  },
                  true,
    ...
    "/api/v1/assets": {
      get: {
        ...
        description:
          "Returns detected Alt Fun supported underlying assets and supported BounceTech Leveraged Tokens from the local LT directory mirror. Spot prices are cached for 10 seconds.",
        parameters: [apiKeyHeader],
        responses: {
          "200": {
                ...
                schema: successResponse({
                  ...
                  properties: {
                      ...
                      items: {
                        ...
                        properties: {
                          ...
                          price: {
                            type: "string",
                            nullable: true,
                            description: "Spot price from Hyperliquid",
                          },
                        },
                      },
                    },
                    leveragedTokens: {
                      type: "array",
                      description:
                        "Leveraged tokens in the LT directory that match Alt Fun's hardcoded supported asset/leverage set.",
                      items: {
                        type: "object",
                        properties: {
                          address: { type: "string" },
                          ...
                          exchangeRate: {
                            type: "string",
                            description: "USD per LT unit (18 decimals)",
                          },
    ...
    "/api/v1/holders/{address}": {
      get: {
        ...
        description:
          "Returns top holders for a token ranked by balance. Computed from trade history.",
        parameters: [
          addressParam("address", "Token contract address"),
          {
            name: "limit",
            ...
            description: "Number of top holders",
          },
          apiKeyHeader,
        ],
        responses: {
              ...
              "application/json": {
                schema: successResponse({
                  ...
                  properties: {
                    ...
                    approximate: {
                      type: "boolean",
                      description: "True if trade history was truncated",
                    },
                  },
                }),
              },
    ...
    "/api/v1/security/{address}": {
      get: {
        ...
        description:
          "Returns security-related information: LP lock status, creator holdings, graduation status.",
        parameters: [
          addressParam("address", "Token contract address"),
          apiKeyHeader,
        ],
              ...
              "application/json": {
                schema: successResponse({
                  ...
                  properties: {
                    lpLocked: {
                      type: "boolean",
                      description: "Whether LP tokens are locked",
                    },
                    lpAmount: {
                      type: "string",
                      nullable: true,
                      description: "LP liquidity amount",
                    },
                    creatorHoldingPct: {
                      type: "number",
                      description:
                        "Creator's current holding as percentage of total supply. Sourced from the indexer's `tokenBalance` index — reflects every ERC-20 Transfer, not just Zap activity.",
                    },
    ...
    "/api/v1/referrals-v2/{wallet}": {
      get: {
        ...
        parameters: [
          addressParam("wallet", "Referrer wallet address"),
          apiKeyHeader,
        ],
        responses: {
          "200": {
              ...
              "application/json": {
                schema: successResponse({
                  ...
                  properties: {
                    referredWallets: {
                      type: "integer",
                      description: "Number of unique referred wallets",
                    },
                    referredVolume: {
                      type: "string",
                      description:
                        "Total USDC volume from referrals (raw, 6 decimals)",
                    },
                    referrals: {
                      type: "array",
                      items: {
    ...
    "/api/v1/images": {
      post: {
        ...
        description:
          "Upload a token image. Accepts JPEG, PNG, GIF, or WebP. Max 5MB. Images are scanned for prohibited content before storage.",
        parameters: [apiKeyHeader],
        requestBody: {
          required: true,
          content: {
        ...
        responses: {
                ...
                schema: successResponse({
                  ...
                  properties: {
                    ...
                    flaggedForReview: {
                      type: "boolean",
                      description:
                        "True if the image was flagged for manual review",
                    },
                  },
                }),
              },
          ...
          "422": {
            description: "Image rejected by content moderation",
            content: { "application/json": { schema: errorResponse } },
          },
          "503": {
            description: "Image moderation unavailable",
            content: { "application/json": { schema: errorResponse } },
          },
        },
      },
    ...
    "/api/v1/images/{prefix}/{key}": {
      get: {
        ...
        description:
          "Serve a previously uploaded image. Responses are cached for 1 year.",
        parameters: [
          {
            name: "prefix",
            in: "path",
    ...
    "/api/v1/moderation/admins/{address}": {
      get: {
        ...
        description:
          "Returns `{ isAdmin: true }` if the supplied wallet is currently configured as a moderation admin. Public endpoint — used by the UI to decide whether to render the admin button. Returns the boolean for one address only; never enumerates the allowlist.",
        parameters: [
          addressParam("address", "Wallet address to check"),
          apiKeyHeader,
        ],
        responses: {
          "200": {
              ...
              "application/json": {
                schema: successResponse({
                  ...
                  properties: {
                    address: {
                      type: "string",
                      description: "Checksummed copy of the requested address.",
                    },
    ...
    "/api/v1/moderation/tokens/{address}/hide": {
      post: {
        ...
        description:
          "Hide a token from public listings. Authenticated by the same 24-hour session signature flow as `PUT /api/v1/profiles/:address` — the admin signs `buildSessionMessage(address, expiresAt)` once per day; the resulting signature is reused for every moderation call. Server recovers the signer via EIP-191, requires the recovered address to match the claimed `address`, and checks that address against the admin allowlist (`ADMIN_WALLETS` env, falling back to `DEFAULT_ADMIN_WALLETS` from `@launchpad/shared`).",
        parameters: [
          addressParam("address", "Token contract address"),
          apiKeyHeader,
        ],
              ...
              schema: {
                ...
                properties: {
                  ...
                  expiresAt: {
                    type: "integer",
                    description:
                      "Unix-ms expiry baked into the signed message; must lie within the session-duration window from now.",
                  },
                },
              },
            },
        ...
        responses: {
          ...
          "400": {
            description: "Invalid address or body",
            content: { "application/json": { schema: errorResponse } },
          },
          "401": {
            description:
              "Signature missing, expired, or not from an admin wallet",
            content: { "application/json": { schema: errorResponse } },
          },
          "404": {
            description: "Token not found in registry",
            content: { "application/json": { schema: errorResponse } },
          },
        },
      },
    ...
    "/api/v1/moderation/tokens/{address}/unhide": {
      post: {
        ...
        description:
          "Restore a hidden token to public listings. Same auth model as `/hide` — see that endpoint for the signature requirements.",
        parameters: [
          addressParam("address", "Token contract address"),
          apiKeyHeader,
        ],
                ...
                schema: successResponse({
                  ...
                  items: {
                    ...
                    properties: {
                      ...
                      classifications: {
                        type: "string",
                        description: "JSON array of {label, score}",
                      },
    ...
    "/api/v1/admin/moderation/logs": {
      get: {
        ...
        description:
          "Returns recent moderation decisions (approved, rejected, pending).",
        security: [{ AdminKeyAuth: [] }],
        parameters: [apiKeyHeader, adminKeyHeader],
        responses: {
          "200": {
      ...
      post: {
        ...
        responses: {
          "200": {
            ...
            content: {
              "application/json": {
                schema: successResponse({ type: "object" }),
              },
            },
          },
          "400": {
    ...
    "/api/v1/webhook/indexer": {
      post: {
        ...
        description:
          "Receives events from the Ponder indexer and broadcasts them to WebSocket subscribers. Requires admin key.",
        security: [{ AdminKeyAuth: [] }],
            ...
            "application/json": {
              schema: {
                ...
                properties: {
                  event: {
                    type: "string",
                    enum: ["trade", "newToken", "graduation", "price", "stats"],
                  ...
                  tokenAddress: {
                    type: "string",
                    description:
                      "Optional token address for token-specific events",
                  },
                },
              },
            },
          ...
          "200": {
            ...
            content: {
              "application/json": {
                schema: successResponse({
                  type: "object",
                  properties: { broadcasted: { type: "boolean" } },
                }),
              },
            },
          },
