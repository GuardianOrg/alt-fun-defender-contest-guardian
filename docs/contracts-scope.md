...
2. If the curve token is already graduatable (LT appreciation pushed it past the threshold), the sell triggers graduation instead of executing — `Zap.sell` calls `Bonding.triggerGraduation` and returns `0`, leaving the seller's tokens untouched so they can exit on the graduated pool. Otherwise, if on curve: routes through `Bonding.sell()` (the internal AMM `Router.sol`). If graduated: swaps on HyperSwap V2 (which charges a 0.3% LP fee on the swap leg, on top of the Alt Fun fee deducted below).
