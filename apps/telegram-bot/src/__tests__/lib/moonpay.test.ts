 ...
 *   ?apiKey=pk_test_abc123&currencyCode=usdc_arbitrum&walletAddress=0x1234…5678
 ...
 */
const EXPECTED_SIGNATURE_USDC_ARBITRUM =
  "B6V84mJepN6Vpm1ZSSKwCCcE7JLtnNtOtMDHIslu4mY=";
...
describe("buildMoonPayBuyUrl", () => {
  it("builds a buy.moonpay.com URL with apiKey + currencyCode + walletAddress + signature", async () => {
    ...
    expect(parsed.searchParams.get("currencyCode")).toBe("usdc_arbitrum");
  ...
  it("appends an HMAC-SHA256(secret, query) base64 signature MoonPay can verify", async () => {
    ...
    expect(parsed.searchParams.get("signature")).toBe(
      EXPECTED_SIGNATURE_USDC_ARBITRUM,
    );
  });
...
describe("resolveBuyUsdcUrl", () => {
  it("uses BUY_USDC_URL verbatim when explicitly set", async () => {
    const url = await resolveBuyUsdcUrl(
  ...
  it("builds a signed MoonPay URL defaulting to usdc_arbitrum when both keys are set", async () => {
    const url = await resolveBuyUsdcUrl(
      {
        MOONPAY_API_KEY: API_KEY,
        MOONPAY_SECRET_KEY: SECRET,
  ...
  it("prefers BUY_USDC_URL over MoonPay when both are set", async () => {
    const url = await resolveBuyUsdcUrl(
      {
        BUY_USDC_URL: "https://override.example/funding",
        MOONPAY_API_KEY: API_KEY,
