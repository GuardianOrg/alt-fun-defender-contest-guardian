 ...
 * block React renders. We start `navigator.hardwareConcurrency` of these
 * per launch and let them race — the first to find a salt whose predicted
 * clone address ends with `suffix` (lowercase hex literal) wins; the hook
 * tears down the rest. The on-chain `Bonding._checkVanity` enforces the
 * same predicate (low-bits mask), so any salt this worker emits is
 * guaranteed to be accepted by `launch`.
 *
 * Design notes:
 ...
 *   - Expected attempts: ~16^suffixLen. With `suffix = "00000"` (the
 *     production value) that's ~1,048,576 — sub-second on a typical
 *     multi-core machine; a few seconds on low-core devices. The host
 *     shows a "FINDING YOUR ADDRESS…" state for the user-visible tail.
 */
...
interface InitMessage {
  ...
  suffix: string; // lowercase hex, e.g. "00000"
  workerIndex: number;
  workerCount: number;
}
...
self.addEventListener("message", (event: MessageEvent<WorkerInbound>) => {
  ...
  while (!stopped) {
    ...
    // address is the last 20 bytes (40 hex chars) of `addrHash` — slice
    // the trailing chars directly rather than materialising the whole
    // address string per attempt.
