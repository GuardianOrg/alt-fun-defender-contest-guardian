...
`GET /ws?channel=<name>&token=<addr?>&apiKey=<key?>` — the upgrade path. `channel` is required; `token` selects the token shard (omit for the wildcard shard, mandatory for global channels like `newToken`/`stats`). The frontend's `WebSocketClient` (`apps/web/src/services/websocket.ts`) multiplexes one underlying WS per subject the app cares about, so the public `subscribe(channel, handler, token?)` surface is unchanged.
